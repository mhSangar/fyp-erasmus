...
def load_img (img_path, label, img_size, batch_size, num_threads, num_epochs=1):
	
	img_t = ops.convert_to_tensor([img_path], dtype=tf.string)
	label_t = ops.convert_to_tensor([label], dtype=tf.int32)

	input_tf_queue = tf.train.slice_input_producer((img_t, label_t), num_epochs=num_epochs)
	...
	img_batch, label_batch = tf.train.batch_join([[img, label]], 
												batch_size=batch_size,
												capacity=4*num_threads, 
												enqueue_many=False,
												allow_smaller_final_batch=True)
