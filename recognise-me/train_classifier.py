...
def load_graph_model(model_path):
	# maybe abspath also works
	model_path_ex = os.path.expanduser(model_path)
	if os.path.isfile(model_path_ex):
		logging.info("Model filename: ".format(model_path_ex))
		with gfile.FastGFile(model_path_ex, "rb") as f:
			# create a tf graph
...
def eval_classifier(embeddings_arr, labels_arr, classifier_filename):
...
def main(input_dir, model_path, classifier_output_path, batch_size, 
	num_threads, num_epochs, split_ratio, training=True):
	...
	with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
		...
		if training:
			...
			eval_classifier(embeddings_arr, labels_arr, classifier_output_path)

		logging.info("Completed in {} secs".format(time.time() - start_time))
