...
\section{Testing}
Two approaches are possible to perform the testing: automated, where a set of unit tests are executed to obtain a "passed"/"failed" answer, and manual testing, where is a human who applies the tests to the system. The automated testing of the GUI is actually possible (e.g. \cite{maveryx_testing}), but the manual testing was finally chosen because its simplicity, validating the requirements defined previously (\ref{sec:requirements}) to verify the proper operation of the system.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{| C{2cm} | C{3cm} | L{9cm} |}
	    \hline
	    \textbf{ID} & \textbf{Implemented} & \multicolumn{1}{c|}{\textbf{Notes}} \\
	    \hline
	    FR{\_}01 & \checkmark & notes \\
	    \hline
	    NFR{\_}01 & \checkmark & notes \\
	    \hline
	    FR{\_}01 & \checkmark & notes \\
	    \hline
	    FR{\_}01 & \checkmark & notes \\
	...
	\caption{Requirement validation}
    \label{table:req_validation}
\end{table}
