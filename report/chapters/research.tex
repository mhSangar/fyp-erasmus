...
uses relu. explain why to use relu. refer to russian paper. paper which initiated the cnn fever.

convolution, after pooling and repeat a number of times.. include image from lecun fig2. pooling helps to detect the combination of the learned features (reference). 

after these stages, a full connected network. not many layers are needed (it would be a pain to train, as said before). 

finally, even a beyesian network would be fine to output the option with higher percenteage. backpropagation used to train weights and filters.

origin of the idea comes from simple_and_complex cells in visual neuroscience. refer to lecun the god. 
